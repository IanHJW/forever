window.onload = function(){
        var ifm= document.getElementById("lframe");
...
//        CanvasParticle(config);
        ...
        for(var i=0;i<li.length-1;i++){
            ...
            li[i].onmousemove = function(){
                ...
                if(litter<=1366){
                    startmove1(bg,this.offsetLeft+(bgleft)/1.7);
                }
                if(litter>1366&&litter<=1920){
                    startmove1(bg,this.offsetLeft+(bgleft)/1.85);
                }
        ...
        for(var i=0;i<li.length-1;i++){
            ...
            li1[i].onmousemove = function(){
                ...
                if(litter<=1366){
                    startmove2(bg2,this.offsetLeft+(bgleft1)/1.7);
                }
                if(litter>1366&&litter<=1920){
                    startmove2(bg2,this.offsetLeft+(bgleft1)/1.55);
                }
            }
            li1[i].onmouseout = function(){
                child1.style.color = 'white';
                child4.firstChild.style.color = '#ea5748';
                if(litter<=1366){
